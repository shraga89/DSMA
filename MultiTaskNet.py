...
def build_multi(first):
    ...
    adnev_in = Sequential()(x)
    adnev = Conv1D(first, kernel_size=4, padding="same", kernel_initializer='ones')(adnev_in)
    adnev = BatchNormalization(momentum=0.8)(adnev)
    adnev = Activation("relu")(adnev)
    adnev = UpSampling1D()(adnev)
    adnev = Conv1D(first, kernel_size=4, padding="same")(adnev)
    adnev = BatchNormalization(momentum=0.8)(adnev)
    adnev = Activation("relu")(adnev)
    adnev = Conv1D(1, kernel_size=4, padding="same")(adnev)
    adnev = MaxPooling1D(padding="same", pool_size=2)(adnev)
    ad = GRU(first * 2, return_sequences=True)(adnev)
    ad_out = TimeDistributed(Dense(1, activation='sigmoid'), name='ad_out')(ad)
    ev = GRU(first*2, return_sequences=False)(adnev)
    ev_out = Dense(1, activation='sigmoid', name='ev_out')(ev)
