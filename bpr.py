...
class BPR(object):
    ...
    def update_factors(self,u,i,j,update_u=True,update_i=True):
        ...
        x = self.item_bias[i] - self.item_bias[j] \
            + np.dot(self.user_factors[u,:], (self.item_factors[i,:]-self.item_factors[j,:]).T.reshape(self.user_factors[u,:].shape))
        ...
        if update_u:
            d = ((self.item_factors[i,:]-self.item_factors[j,:])*z - self.user_regularization*self.user_factors[u,:]).T.reshape(self.user_factors[u,:].shape)
    ...
    def loss(self):
        ...
        for u,i,j in self.loss_samples:
            ...
            complexity += self.positive_item_regularization * np.dot(self.item_factors[i],self.item_factors[i].T)
            complexity += self.negative_item_regularization * np.dot(self.item_factors[j],self.item_factors[j].T)
    ...
    def predict(self,u,i):
        #print(self.user_factors[u].shape)
        #print(self.item_factors[i].T.reshape(self.user_factors[u].shape))
        return self.item_bias[i] + np.dot(self.user_factors[u],self.item_factors[i].T.reshape(self.user_factors[u].shape))
        ...
        if self.sample_negative_items_empirically:
            ...
            if len(self.data[u].indices)==0: 
               i = random.randint(0,self.num_items-1)
                ...
                i = np.random.choice(len(self.data[u].indices), 1)
        else:
            i = random.randint(0,self.num_items-1)
        return i
...
class UniformUserUniformItem(Sampler):
    ...
    def generate_samples(self,data,max_samples=None):
        ...
        for _ in range(self.num_samples(self.data.nnz)):
            u = self.uniform_user()
            if len(self.data[u].indices) == 0: break
    ...
    def generate_samples(self,data,max_samples=None):
        ...
        for _ in range(self.num_samples(self.data.nnz)):
            ...
            if len(user_items) == 0:
                ...
                user_items = self.local_data[u].nonzero()[1]
            i = user_items[int(random.random() * len(user_items))]
